...
from src.models.deep_markov_model.model import calculate_loss, DMM
...
noise_variance = 0.1
random_seed = 42
MODEL_SAVE_PATH = 'dmm_model_weights.pth'  # Define a path for the saved weights

# --- Hyperparameters for the DMM Model ---
latent_dim = 3
hidden_dim = 64
obs_dim = 10
num_epochs = 2000
...
# --- Training Loop ---
print("\nStarting DMM training...")
model = DMM(obs_dim, latent_dim, hidden_dim).to(device)
...
loaded_model = DMM(obs_dim, latent_dim, hidden_dim).to(device)
...
ax2.set_title("Inferred Latent Dynamics (DMM)")
ax2.set_xlabel("X");
ax2.set_ylabel("Y");
ax2.set_zlabel("Z")
