...
class StationaryKernel(nn.Module):
    ...
    def scaled_paired_diff(self, x1, x2, cov):
        ...
        res1 = torch.linalg.solve(
            cov.unsqueeze(-3),  # (..., D, D) coefficient matrix
            diff.transpose(-2, -1)  # (..., D, N1*N2) RHS
        )
...
class RBF(StationaryKernel):
    ...
    def Psi2(self, x2, mu, cov, x3=None):
        ...
        mu_x2_new_diff_scaled = torch.linalg.solve(
            lengthscales_new_squared.unsqueeze(-3),  # A (..., D, D)
            mu_x2_new_diffs.transpose(-1, -2)  # B (..., D, N)
        )
    ...
    def dPsi2dmu(self, x2, mu, cov, x3=None):
        ...
        #scaled_diffs, _ = torch.linalg.solve(mu_x2_new_diffs.transpose(-1, -2), lengthscales_new_squared.unsqueeze(-3))

        scaled_diffs = torch.linalg.solve(
            lengthscales_new_squared.unsqueeze(-3),  # A (..., K, K)
            mu_x2_new_diffs.transpose(-1, -2)  # B (..., K, M*M)
        )
    ...
    def dPsi2dcov(self, x2, mu, cov, x3=None):
        ...
        scaled_diffs = torch.linalg.solve(
            lengthscales_new_squared.unsqueeze(-3),  # A (..., K, K)
            mu_x2_new_diffs.transpose(-1, -2)  # B (..., K, N)
        )
    ...
    def Psid1Psi1(self, x2, mu, cov, x3=None):
        ...
        scaled_diffs = torch.linalg.solve(
            new_cov.unsqueeze(-3).unsqueeze(-3),  # A (..., K, K)
            mu_x2_new_diffs.unsqueeze(-1)  # B (..., K, 1)
        )  # R x M2 x M3 x K x 1
    ...
    def Psid1d2(self, x2, mu, cov):
        ...
        AinvSigmaAinv = torch.linalg.solve(
            new_cov,
            cov.div(self.lengthscales.pow(2).view(1, -1))
        )
