    ...
    def closedFormUpdates(self, mq, Sq, Aq, bq, trLen, wwLeg):
        ...
        if self.useClosedForm:
            with torch.no_grad():
                ...
                Anew = torch.linalg.solve(
                    AutoCorr.transpose(-2, -1),  # A (..., n, n)
                    (XCorr + OffsetCorr).transpose(-2, -1)  # B (..., n, k)
                )
...
class SparseGP(TransitionFunction):
    ...
    def closedFormUpdates(self, mq, Sq, Aq, bq, trLen, wwLeg):
        ...
        if self.useClosedForm:
            with torch.no_grad():
                ...
                CovMatinvK = torch.linalg.solve(CovMat, Kzz)
                ...
                q_mu_new = torch.linalg.solve(CovMat, Xcorr1 - Xcorr2)
    ...
    def predict(self, x):
        ...
        Ak = torch.linalg.solve(Kzz, q_mu)
        mean = Kxz.matmul(Ak)
        # pdb.set_trace()
        # Kzz_inv = self.get_Kzz_inv()
    ...
    def ff(self, m, S):
        ...
        # Kzz^{-1} q_mu
        Kzz_inv_U = torch.linalg.solve(Kzz, q_mu.unsqueeze(0))

        # Kzz^{-1} sum(Su)
        Kzz_inv_q_sigmas = torch.linalg.solve(Kzz, q_sigma.sum(0))

        # Kzz^{-1} Kzxxz
        Kzz_inv_Kzxxz = torch.linalg.solve(Kzz, Kzxxz)
    ...
    def dfdS(self, m, S):
        ...
        with torch.no_grad():
            ...
            Ak = torch.linalg.solve(Kzz, Kxz.permute(0, 3, 4, 2, 1))
    ...
    def dffdS(self, m, S):
        ...
        with torch.no_grad():
            ...
            # Kzz^{-1} Kzxxz   (R x (K x K) x (M x M))
            Kzz_inv_Kzxxz = torch.linalg.solve(Kzz, Kzxxz.permute(0, 3, 4, 1, 2))
    ...
    def ddfdxdS(self, m, S):
        ...
        with torch.no_grad():
            ...
            # Kzz_inv_U, _ = torch.solve(q_mu.unsqueeze(0), Kzz)  # R x M x K
            Kzz_inv_U = torch.linalg.solve(Kzz, q_mu.unsqueeze(0))  # R x M x K
    ...
    def dffdm(self, m, S):
        ...
        with torch.no_grad():
            ...
            Kzz_inv_U = torch.linalg.solve(Kzz, q_mu.unsqueeze(0))  # R x M x K
            Kzz_inv_q_sigmas = torch.linalg.solve(Kzz, q_sigma.sum(0))  # Kzz^{-1} sum(Su)

            # Kzz^{-1} Kzxxz
            #Kzz_inv_Kzxxz, _ = torch.solve(Kzxxz.permute(0, 3, 4, 1, 2), Kzz)  # R x (K x K) x (M x M)
            Kzz_inv_Kzxxz = torch.linalg.solve(Kzz, Kzxxz.permute(0, 3, 4, 1, 2))  # R x (K x K) x (M x M)
    ...
    def kullback_leibler(self):
        ...
        Kzzinv_sig = torch.linalg.solve(q_sigma.sum(0), Kzz)
        tt1 = batch_diag(Kzzinv_sig).sum()

        # q_mu' * Kzz_inv * q_mu
        # Kzzinv_q, _ = torch.solve(q_mu, Kzz)
        Kzzinv_q = torch.linalg.solve(Kzz, q_mu)
...
class FixedPointSparseGP(SparseGP):
    ...
    def closedFormUpdates(self, mq, Sq, Aq, bq, trLen, wwLeg):
        ...
        if self.useClosedForm:
            with torch.no_grad():
                ...
                Kcond_inv_Kvec = torch.linalg.solve(Kcond, Kvec.transpose(-1, -2))
                ...
                Kblk_inv_Kzxxz = torch.linalg.solve(Kblk, (Kzxxz * wwLeg.unsqueeze(-1).unsqueeze(-1)).sum(0).sum(0))
                Kblk_inv_Kzxxz_Kblk_inv = torch.linalg.solve(Kblk, Kblk_inv_Kzxxz.transpose(-2, -1))
                ...
                Kblk_inv_Xcorr1 = torch.linalg.solve(Kblk, Xcorr1)
                Kblk_inv_Xcorr2 = torch.linalg.solve(Kblk, Xcorr2)
                # update for q_sigma (same across all dimensions)
                ...
                q_joint_new = torch.linalg.solve(CovMat_joint.squeeze(0),
                                           Kblk_inv_Xcorr1.squeeze(0)[idx, :] - Kblk_inv_Xcorr2.squeeze(0)[idx, :])
    ...
    def kullback_leibler(self):
        ...
        Kblockinv_sig = torch.linalg.solve(Kblock, q_sigma.sum(0))
        ...
        Kcond_inv_q_cond = torch.linalg.solve(Kcond, q_cond)

        p_mu = Kvec.matmul(Kcond_inv_q_cond)

        #KBlockinv_q, _ = torch.solve(q_mu - p_mu, Kblock)
        KBlockinv_q = torch.linalg.solve(Kblock, q_mu - p_mu)
    ...
    def get_Kzz_inv(self):
        ...
        Kdd_inv_Kdz = torch.linalg.solve(Kdd, Kdz)
        Kzz_inv_Kzd = torch.linalg.solve(Kzz, Kdz.transpose(-2, -1))
